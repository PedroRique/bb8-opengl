...
char* filenameTexMetal1 = _strdup("./bodyTexture.bmp");
...
GLfloat lightposition[4] = { 10.0f, 10.0f, 10.0f, 0.0f };
GLfloat luzAmbiente[4] = { 0.15, 0.15, 0.15, 0.0 };
...
void handleClick(int button, int state, int x, int y) {
	...
	if (button == GLUT_LEFT_BUTTON)
		if (state == GLUT_DOWN) {  // Zoom-in
			if (eyeDistance >= 8) eyeDistance -= 1;
		}
...
void handleKeypress(unsigned char key, int x, int y) {
	...
	case '1': //Increase arm angle
		if (angleArm < 180) angleArm += 3;
		glutPostRedisplay();
		break;
	case '2': //Decrease arm angle
		if (angleArm > 0) angleArm -= 3;
		glutPostRedisplay();
		break;
...
void drawScene(void) {
	...
	glTranslatef(0.0f, -5.0f, -5);

	glPushMatrix();

	// move to arm referential
	glRotatef(angleArm, 90.0f, 0.0f, 1.0f);
